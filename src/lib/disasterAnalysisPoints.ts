...
export type StationAnalysisPoint = GeoPoint & {
  label: string;
  context: string;
};

export type StationArea = GeoPoint & {
  id: string | null | undefined;
  name: string;
  coverageRadiusKm?: number | null;
};

export type BoundaryPolygon = number[][][];
...
export const isVietnamLandCoordinate = (latitude: number, longitude: number) => {
...
const clampToVietnamLand = (point: GeoPoint, fallback: GeoPoint): GeoPoint => {
...
const toCoordKey = (point: GeoPoint) =>
  `${point.latitude.toFixed(4)},${point.longitude.toFixed(4)}`;

export const toAnalysisCoordKey = (latitude: number, longitude: number) =>
...
export const findNearestStationArea = (point: GeoPoint, stations: StationArea[]) => {
  let nearest: { station: StationArea; distanceKm: number } | null = null;
  ...
  return nearest as { station: StationArea; distanceKm: number } | null;
};

export const isPointWithinStationArea = (point: GeoPoint, stations: StationArea[]) => {
...
export const getStationFallbackRadiusKm = (station: Pick<StationArea, 'coverageRadiusKm'>) =>
  Math.max((station.coverageRadiusKm || 12) * 1.35, 18);

export const isPointWithinStationFallbackArea = (point: GeoPoint, station: StationArea) =>
  getDistanceKm(point, station) <= getStationFallbackRadiusKm(station);

export const buildStationFallbackBoundary = (
  station: StationArea,
  segments = 48,
): BoundaryPolygon => {
...
const isPointInRing = (point: GeoPoint, ring: number[][]) => {
  let inside = false;
...
export const isPointInBoundaryPolygon = (point: GeoPoint, polygons?: BoundaryPolygon | null) => {
...
export const buildStationAnalysisPoints = (
  ...
  const uniquePoints = new Map<string, StationAnalysisPoint>();
